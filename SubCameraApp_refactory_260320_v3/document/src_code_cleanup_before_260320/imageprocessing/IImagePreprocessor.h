...
public:
    ...
    // 원시 프레임 → AI 모델 입력 텐서 포맷으로 변환 및 직접 쓰기 (Zero-copy)
    // tensor_ptr: TFLite 입력 텐서의 시작 주소
    virtual void preprocessToTensor(const cv::Mat& raw_frame, float* tensor_ptr) = 0;

    // (기존) cv::Mat 반환 버전 (필요 시 유지)
