...
void StreamPipeline::stopStreaming() {
  ...
  // 이미 중지되었더라도, 스레드 객체가 joinable하다면 확실히 정리해야 함
  is_streaming_ = false;
  std::cout << "[StreamPipeline] 스트리밍 중지 시도..." << std::endl;

  // 1. AI 스레드를 깨움 (ThreadSafeQueue 알림)
  frame_queue_.notify_all();

  // 2. 각 스레드가 joinable한지 확인 후 안전하게 종료 대기
  if (camera_thread_.joinable()) {
    std::cout << "[StreamPipeline] 카메라 스레드 종료 대기 중..." << std::endl;
    camera_thread_.join();
  }
  if (ai_thread_.joinable()) {
    std::cout << "[StreamPipeline] AI 스레드 종료 대기 중..." << std::endl;
    ai_thread_.join();
  }
...
void StreamPipeline::aiWorkerLoop() {
  ...
  while (is_streaming_) {
    ...
    auto elapsed_ms = std::chrono::duration_cast<std::chrono::milliseconds>(
                          end_time - start_time)
                          .count();
    std::cout << "[AI Cycle] 실행 시간: " << elapsed_ms << " ms" <<
    std::endl;
  }
}
...
void StreamPipeline::cameraLoop() {
  ...
  while (is_streaming_) {
    ...
    if (AppConfig::ENABLE_DISPLAY) {
      ...
      if (cv::waitKey(1) == 'q') {
        std::cout << "[StreamPipeline] 'q' 키 입력 감지. 종료를 시도합니다..." << std::endl;
        is_streaming_ = false;
