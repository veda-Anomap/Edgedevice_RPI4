  ...
  if (body.find("\"motor\"") != std::string::npos || body.find("\"cmd\"") != std::string::npos) {
    if (edge_bridge_) {
      ...
      try {
        ...
        if (j.contains("motor") && j.at("motor").is_string()) motor_cmd = j.at("motor");
        else if (j.contains("cmd") && j.at("cmd").is_string()) motor_cmd = j.at("cmd");

        if (!motor_cmd.empty()) {
          edge_bridge_->handleMotorCmd(motor_cmd);
        }
      } catch (...) {}
    }
  }
