...
public:
  Mat apply(const Mat &src, float strength) override {
    ...
    // [수정] 강도를 5.0f로 낮춰 1차적 링잉 임계값 조절 (트랙바 개입 여지 복구)
    float auto_strength = strength * (float)mean_blur[0] * 5.0f; 

    Mat laplacian, result;
    Laplacian(src, laplacian, CV_16S, 3);
    
    // [신규 논문 응용] 링잉 방지(Ringing-Free)를 위한 ASOC (로컬 클램핑)
    // 원본 픽셀의 3x3 주변 최소/최대값을 넘지 않도록 가둬서 수학적으로 링잉 파동을 차단
    Mat min_v, max_v;
    erode(src, min_v, getStructuringElement(MORPH_RECT, Size(3, 3)));
    dilate(src, max_v, getStructuringElement(MORPH_RECT, Size(3, 3)));
    ...
    for (int y = 0; y < src.rows; y++) {
      for (int x = 0; x < src.cols; x++) {
        ...
        for (int c = 0; c < 3; c++) {
          ...
          float sharp_val = orig - (laplacian.at<Vec3s>(y, x)[c] * weight * auto_strength);
          
          // 로컬 경계선(Min/Max) 클램핑 -> 링잉 증발!
          float min_bound = min_v.at<Vec3b>(y, x)[c];
          float max_bound = max_v.at<Vec3b>(y, x)[c];
          sharp_val = std::max(min_bound, std::min(sharp_val, max_bound));
          
          result.at<Vec3b>(y, x)[c] = saturate_cast<uchar>(sharp_val);
        }
      }
    }
    return result;
  }
  string getName() const override { return "Gradient Ratio (Auto DFD)"; }
  string getBrief() const override { return "Ringing-Free Blur Map Auto"; }
};
...
public:
  Mat apply(const Mat &src, float strength) override {
    ...
    // [신규 논문 응용] 화질 훼손(Ghosting/고스팅) 아티팩트 박멸을 위한 ASOC 클램핑
    Mat min_v, max_v;
    erode(src, min_v, getStructuringElement(MORPH_RECT, Size(3, 3)));
    dilate(src, max_v, getStructuringElement(MORPH_RECT, Size(3, 3)));
    ...
    for (int y = 0; y < src.rows; y++) {
      for (int x = 0; x < src.cols; x++) {
        ...
        if (w > 0.05f) {
            for (int c = 0; c < 3; c++) {
              float orig = src.at<Vec3b>(y, x)[c];
              float raw_lap = laplacian.at<float>(y, x);
              float sharp_val = orig - (raw_lap * w * strength * 2.0f);
              
              // 잔상(고스팅) 제거 클램핑! 주변 3x3 픽셀을 넘어서는 튀는 값 강제 절단
              float min_bound = min_v.at<Vec3b>(y, x)[c];
              float max_bound = max_v.at<Vec3b>(y, x)[c];
              sharp_val = std::max(min_bound, std::min(sharp_val, max_bound));
  ...
  string getBrief() const override { return "Ghosting-Free Event Guided"; }
};
